	...
	switch (req->bRequest) {
		...
		case USB_CDC_REQ_GET_LINE_CODING: {
			...
			if ( *len < sizeof(struct usb_cdc_line_coding) ) {
				debug_println("*** cdcacm_control notsupp line_coding "); debug_print(sizeof(struct usb_cdc_line_coding)); 
				debug_println(", len "); debug_print(*len);
...
cdcacm_data_rx_cb(
  ...
  uint8_t ep __attribute__((unused))
    ...
    debug_println("["); debug_println(cdcbuf); debug_println("]"); // debug_flush(); ////
}
