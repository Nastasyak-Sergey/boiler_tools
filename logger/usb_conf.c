...
static const char *usb_strings[] = {
    ...
    "Black Pill DATA",          //  DATA
};

#define MSC_VENDOR_ID "BlackPill"  //  Max 8 chars
#define MSC_PRODUCT_ID "Data Logger"  //  Max 16 chars
...
void dump_usb_request(const char *msg, struct usb_setup_data *req) {
    ...
    debug_println(msg);
    debug_println(" typ "); debug_print_hex(req->bmRequestType);
    debug_println(", req "); debug_print_hex(req->bRequest);
    debug_println(", val "); debug_print_hex(req->wValue >> 8); debug_print_hex(req->wValue & 0xff);
    debug_println(", idx "); debug_print_hex(req->wIndex >> 8); debug_print_hex(req->wIndex & 0xff);
    debug_println(", len "); debug_print_hex(req->wLength >> 8); debug_print_hex(req->wLength & 0xff);

    if (req->bmRequestType == 0x00 || req->bmRequestType == 0x80) {
        //  Dump USB standard requests.
        if (req->bmRequestType == 0x80 && req->bRequest == 0x06) {
            debug_println(", GET_DES");
            switch(desc_type) {
                case 1: debug_println("_DEV"); break;
                case 2: debug_println("_CFG"); break;
                case 3: debug_println("_STR"); break;
                case 4: debug_println("_INF"); break;
                case 5: debug_println("_ENP"); break;
                case 15: debug_println("_BOS"); break;
            }
            ...
            debug_println(", SET_CFG    ");
        } else if (req->bmRequestType == 0x80 && req->bRequest == 0x09) {
            debug_println(", SET_CFG    ");
        } else {
            debug_println(",");
        }
        debug_println(" t "); debug_print_hex(desc_type); 	
        debug_println(" i "); debug_print_hex(desc_index); 	
    }
    debug_println("\n");
}
