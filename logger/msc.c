...
#endif  //  NOTUSED				
				default:
					debug_println("scsi_inquiry notsup "); debug_print_hex(page_code); debug_println(""); debug_flush(); ////
			}
		}
	}
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	default:
        debug_println("SBC_SENSE_KEY_ILLEGAL_REQUEST "); debug_print_hex(trans->cbw.cbw.CBWCB[0]); debug_println(""); debug_flush(); ////
	...
	if (0 < left) {
		...
		if (sizeof(struct usb_msc_cbw) == trans->cbw_cnt) {
...
#ifdef NOTUSED
            debug_println("msc_data_rx_cb byte_count "); 
            debug_print(trans->byte_count);
            debug_println(", bytes_to_read "); 
            debug_print(trans->bytes_to_read);
            debug_println(", CBWCB "); 
            debug_print_hex(trans->cbw.cbw.CBWCB[0]);
			...
			if (trans->byte_count < trans->bytes_to_read) {
				 ...
				 * read again. */
#ifdef NOTUSED				
                debug_println("msc_data_rx_cb wait byte_count "); 
                debug_print(trans->byte_count);
                debug_println(", bytes_to_read "); 
                debug_print(trans->bytes_to_read);
                debug_println(""); // debug_flush(); ////
