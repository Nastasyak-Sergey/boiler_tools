...
uint16_t w25_get_capacity (uint32_t spi) {
    //    "W25X16",   // 14
    //    "W25X32",   // 15
    //    "W25X64"    // 16
    uint32_t info; 
    info = w25_JEDEC_ID(spi);

    return (uint16_t)(info&0xFF);           
}
 ...
 *********************************************************************/
		// New address is returned
uint32_t  w25_write_data(uint32_t spi,uint32_t addr,void *data,uint32_t bytes) {
	uint8_t *udata = (uint8_t*)data;
