...
//    printf("usb_setup num_strings %d \n", num_strings);
    ...
    if (status < 0) { printf("*** usb_setup failed *** \n"); }
...
int aggregate_register_config_callback( usbd_device *usbd_dev, usbd_set_config_callback callback) {
    ...
    printf("*** ERROR: Too many config callbacks ***\n"); 
	return -1;
}
...
int aggregate_register_callback(
    ...
    printf("*** ERROR: Too many control callbacks *** \n");
	return -1;
}
...
void dump_usb_request(const char *msg, struct usb_setup_data *req) {
    ...
    printf("%s ", msg);
    printf(" typ %x", req->bmRequestType);
    printf(", req %x",req->bRequest);
    printf(", val %x%x ", (req->wValue >> 8), (req->wValue & 0xff));
    printf(", idx %x%x ", (req->wIndex >> 8), (req->wIndex & 0xff));
    printf(", len %x%x ", (req->wLength >> 8), (req->wLength & 0xff));
    ...
    if (req->bmRequestType == 0x00 || req->bmRequestType == 0x80) {
        ...
        printf(" type %x", desc_type); 	
        printf(" index %x", desc_index); 	
    }
    printf("\n");
}
