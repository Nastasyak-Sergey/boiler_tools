...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	default:
        printf("SBC_SENSE_KEY_ILLEGAL_REQUEST %x ", trans->cbw.cbw.CBWCB[0]);
	...
	if (0 < left) {
		...
		if (sizeof(struct usb_msc_cbw) == trans->cbw_cnt) {
			...
			if (trans->byte_count < trans->bytes_to_read) {
				 ...
				 * read again. */
#ifdef NOTUSED				
                printf("msc_data_rx_cb wait byte_count %d \n", trans->byte_count);
                printf(", bytes_to_read %d \n", trans->bytes_to_read);
#endif  //  NOTUSED				
		...
		if (0 < trans->block_count) {
			if (0 == (0x1ff & trans->byte_count)) {
				...
				if (0 != (*ms->write_block)(lba, trans->msd_buf)) {
					/* Error */
                    printf("msc_data_rx_cb write error \n");
				}
				trans->current_block++;
			}
		}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if (0 < trans->block_count) {
			...
			if (0 == (0x1ff & trans->byte_count)) {
				...
				if (0 != (*ms->read_block)(lba, trans->msd_buf)) {
					/* Error */
                    printf("msc_data_rx_cb read error \n");
				}
				trans->current_block++;
			}
		}
		...
		if (0 < trans->block_count) {
			if (trans->current_block == trans->block_count) {
				...
				if (0 != (*ms->write_block)(lba, trans->msd_buf)) {
					/* Error */
                    printf("msc_data_rx_cb write error 2 \n");; ////
				}
	...
	if (trans->byte_count < trans->bytes_to_write) {
		if (0 < trans->block_count) {
			if (0 == (0x1ff & trans->byte_count)) {
				...
				if (0 != (*ms->read_block)(lba, trans->msd_buf)) {
					/* Error */
                    printf("msc_data_tx_cb read error\n"); ////
				}
				trans->current_block++;
			}
		}
				 ...
				 uint8_t msc_interface_index0)  //  Index of MSC interface
{
	...
	if (status < 0) { printf("*** custom_usb_msc_init failed ***\n"); }
